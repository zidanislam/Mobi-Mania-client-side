...
const MyOrder = () => {
  ...
  useEffect(() => {
    fetch(`https://radiant-brushlands-53565.herokuapp.com/allorders?email=${email}`)
      .then((res) => res.json())
      .then((result) => {
        setmyOrder(result);
      });
  ...
  const handelDeletMyorder = (id) => {
    ...
    if (result) {
      fetch(`https://radiant-brushlands-53565.herokuapp.com/allorder/${id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <div>
      <h1 className="my-4">My All Order list</h1>
